...
function renderPagination() {
  ...
  let html = `
    <div style="display:flex; justify-content:center; align-items:center; gap:12px; margin:40px 0;">
      <button class="btn btn-primary" onclick="changePage(${currentPage - 1})" ${currentPage <= 1 ? 'disabled' : ''} style="padding:8px 16px; border-radius:8px;">السابق</button>
      <div style="font-weight:600; color:#475569;">صفحة ${currentPage} من ${totalPages}</div>
      <button class="btn btn-primary" onclick="changePage(${currentPage + 1})" ${currentPage >= totalPages ? 'disabled' : ''} style="padding:8px 16px; border-radius:8px;">التالي</button>
    </div>
  `;
  nav.innerHTML = html;
}
    ...
    <div class="store-product-card" style="display:flex;flex-direction:column;">
      <a href="${productLink}" style="display:block; text-decoration:none; color:inherit; flex:1;">
        <div class="store-product-img" style="position:relative">
          ${img ? `<img src="${img}" alt="${p.name}" style="width:100%;height:100%;object-fit:contain" ${lazyAttr} decoding="async" onerror="this.style.display='none'">` : ''}
